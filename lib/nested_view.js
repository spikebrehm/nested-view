...
(function(){
  ...
  var _ = glob._ || require('underscore');
  var Backbone = glob.Backbone || require('backbone');
  var Handlebars = glob.Handlebars || require('handlebars');
  ...
  var NestedView = Backbone.View.extend({
    ...
    getTemplate: function() {
      return JST[ this.template || this.name ];
    },
  ...
  NestedView.viewHelper = function(context, options) {
    ...
    var viewOptions = NestedView._getViewOptions( context, options, parentView );
  ...
  NestedView.dynviewHelper = function( context, options ) {
    var viewName = context,
        parentView = this._view,
        View, view, html;
    ...
    if ( !parentView ) {
      throw new Error( 'No parent view defined for dyamic view' );
    }

    // Assign the dynamic views object and check if it's there.
    var dynamicViews = parentView.dynamicViews;
    if ( !_.isObject( dynamicViews ) ) {
      throw new Error( '"dynamicViews" parameter not defined in parent view');
    }

    var viewOptions = NestedView._getViewOptions( context, options, parentView );

    View = dynamicViews[viewName];
    view = new View(viewOptions);
  ...
  NestedView._getViewOptions = function ( context, options, parentView ) {
    var viewOptions = options.hash || {};


    // Allow passing in context with special 'context' key.
    if (viewOptions.context) {
      viewOptions = viewOptions.context;
    } else if ( !_.isEmpty( parentView.options )) {

      // pass any context the view has
      viewOptions = _.extend( viewOptions, parentView.options );
    }
  ...
  NestedView.registerHandlebars = function(Handlebars) {
    ...
    Handlebars.registerHelper('dynview', NestedView.dynviewHelper);
  };
